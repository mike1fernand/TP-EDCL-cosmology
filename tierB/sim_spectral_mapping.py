...
def run(params: Params) -> dict:
    ...
    l1 = float(_TRAPZ(np.abs(S_t[band] - S_pred[band]), x=w_t[band]) / np.maximum(_TRAPZ(S_t[band], x=w_t[band]), 1e-12))
    ...
    l1_inh = float(_TRAPZ(np.abs(S_inh[band2] - S_pred_inh[band2]), x=w_inh[band2]) / np.maximum(_TRAPZ(S_inh[band2], x=w_inh[band2]), 1e-12))
